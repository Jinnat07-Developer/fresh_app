                ...
                <div class="card-body">
                    ...
                    <form class="form-horizontal" action="{{route('unit.update',['id'=>$unit->id])}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-9">
                                <input class="form-control" id="unitName" value="{{$unit->name}}" type="text" name="name">
                            </div>
                        </div>
                        <div class="row mb-4">
                            <label for="description" class="col-md-3 form-label">Unit Description</label>
                            <div class="col-md-9">
                                <textarea class="form-control"  placeholder="Enter your description" name="description" type="text">{{$unit->description}}</textarea>
                            </div>
                        </div>
                        <div class="row mb-4">
                            <label for="code" class="col-md-3 form-label">Unit code</label>
                            <div class="col-md-9">
                                <input class="form-control" id="code" value="{{$unit->code}}" type="text" name="code">
                            </div>
                        </div>
                        <div class="row mb-4">
                            <label for="status" class="col-md-3 form-label">Unit Status</label>
                            <div class="col-md-9">
                                <label><input type="radio" id="status" value="1" name="status" {{$unit->status == 1 ? 'checked': ''}}>Published</label>
                                <label><input type="radio" id="status" value="0" name="status" {{$unit->status == 0 ? 'checked': ''}}>UnPublished</label>
                            </div>
                        </div>
